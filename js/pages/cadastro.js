async function cadastrarItem() {
  ...
  const result = await ItemService.create({ codigo, nome, categoria, localizacao, quantidade, estoque_minimo, observacao });
  if (!result.success) { showMsg('cad-msg','error',result.error); return; }
...
function limparCadastro() {
  ['cad-codigo','cad-nome','cad-categoria','cad-local','cad-obs'].forEach(id => document.getElementById(id).value='');
