...
async function salvarNovaSenha() {
  ...
  if (nova !== confirm) {
    document.getElementById('pwd-modal-msg').innerHTML=`<div class="alert-box" style="margin-bottom:0">❌ As senhas não coincidem!</div>`;
    return;
  }
  const result = await UserService.changePassword(id, nova);
  if (!result.success) {
    document.getElementById('pwd-modal-msg').innerHTML=`<div class="alert-box" style="margin-bottom:0">❌ ${result.error}</div>`;
    return;
  }
  closePwdModal();
