async function renderHistorico() {
  ...
  if (!filtered.length) {
    tbody.innerHTML=`<tr><td colspan="6"><div class="empty-state"><div class="empty-icon">📋</div><div>${enriched.length?'Nenhum registro com esses filtros.':'Nenhuma movimentação ainda.'}</div></div></td></tr>`;
    return;
  }
  tbody.innerHTML = filtered.map(m=>`<tr>
    <td style="color:var(--muted);white-space:nowrap">${fmtDate(m.data)}</td>
    <td><strong>${esc(m.item_nome)}</strong><br><span style="font-size:11px;color:var(--muted)">${esc(m.item_codigo)}</span></td>
    ...
    <td style="color:var(--muted)">${esc(m.observacao)||'—'}</td>
  </tr>`).join('');
}
